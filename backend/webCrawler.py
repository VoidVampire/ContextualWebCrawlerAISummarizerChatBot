...
class RelevantWebCrawler:
    ...
    async def crawl(self, seed_urls: List[str], original_content: str):
        ...
        if not valid_seeds:
            return [], 0, 0  # Return empty results if no valid seed URLs
        ...
        async with aiohttp.ClientSession() as session:
            ...
            while (time.time() - start_time < 30 and  # 30-second timeout
                ...
                if not workers and self.url_queue.empty() and not self.relevant_pages:
                    return [], len(self.visited_urls), 0  # No relevant links found
        ...
        return [page['url'] for page in sorted_pages]
...
async def get_relevant_links_async(seed_urls: List[str], original_content: str) -> List[str]:
    """Async helper function for getting relevant links."""
    try:
        crawler = RelevantWebCrawler()
        relevant_links = await crawler.crawl(seed_urls, original_content)
        return relevant_links
    except Exception as e:
        logger.error(f"Crawler error: {str(e)}")
        return ["An error occurred while searching for relevant links."], 0, 0

async def get_relevant_links(seed_urls: List[str], original_content: str) -> List[str]:
    """Unified async entry point for getting relevant links."""
    return await get_relevant_links_async(seed_urls, original_content)
